...
CREATE TABLE MEMBER(
	...
	MEMBER_NAME VARCHAR2(15), 		 -- 한글 2~5자 * 3 bytes
	MEMBER_SSN  CHAR(14), 	  		 -- 123456-7891234 (항상 14글)
	ENROLL_DATE DATE DEFAULT SYSDATE -- 가입일
);
-- DEFAULT : INSERT/UPDATE 시 해당 컬럼의 값을 입력하지 않거나, DEFAULT를 작성하면 해당 값(SYSDATE)이 컬럼에 반영된다.
...
COMMENT ON COLUMN MEMBER.ENROLL_DATE IS '회원 가입';

-- USER_TABLES : 사용자가 작성한 테이블을 확인 하는 뷰
-- 데이터 딕셔너리에 정의되어 있음
-- 딕셔너리 뷰(데이터 딕셔너리에서 일부만 뽑아 조회하는 가상 테이블)
SELECT * FROM USER_TABLES;

-- MEMBER 테이블에 샘플 데이터 삽입
INSERT INTO MEMBER VALUES('MEM01', '123ABC', '홍길동', '980418-1234567', SYSDATE);
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02', 'QWER1234', '김영희', '920314-2234567', SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES('MEM03', '1Q2W3E4R', '김철수', '941231-1234567', DEFAULT);

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME, MEMBER_SSN) 
VALUES('MEM04', '123ABC', '이지연', '000101-4234567');

--  데이터  삽입 확인
SELECT * FROM MEMBER;
COMMIT;   -- TCL : 트랜잭션에 저장된 DML 구문을 DB에 반영
ROLLBACK; -- 마지막 COMMIT 상태로 돌아감(트랜잭션에 저장된 DML 삭제)

--------------------------------------------------------------------------------------------------------------------

/* 주민등록번호, 전화번호와 같이 숫자로만 이루어진 데이터를 NUMBER 타입으로 지정할 경우의 문제점 */
...
INSERT INTO MEMBER2 VALUES('고길동', 8802031234567, 01059239363);
INSERT INTO MEMBER2 VALUES('김길동', 0002031234567, 01059239363);

SELECT * FROM MEMBER2;
--> 8802031234567, 1059239363
--> 2031234567, 1059239363
--> NUMBER타입의 컬럼은 값 제일 앞 0을 유지하지 않는 문제 발생
--  => 해결방법 : CHAR, VARCHAR2 타입으로 컬럼 타입을 지정
...
-- ORA-01400: NULL을 ("KH_CYN"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- UNIQUE 제약 조건 테이블 생성
CREATE TABLE USER_USED_UK( -- (UNIQUE KYE)
    USER_NO NUMBER,
--    USER_ID VARCHAR2(20) UNIQUE, --> (1) 컬럼레벨 제약조건 설정(이름 미지정)
--    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_UK UNIQUE, --> (2) 컬럼레벨 제약조건 설정(이름 지정: USER_ID_UK)
    USER_ID VARCHAR2(20),
    USER_PWD VARCHAR2(30),
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    ...
    -- 테이블레벨 제약조건 설정
--    , UNIQUE(USER_ID) -- (3) 제약조건(컬럼명) (이름 미지정)
    , CONSTRAINT USER_ID_UK UNIQUE(USER_ID) -- (4) 이름 지정
);
...
-- ORA-00001: 무결성 제약 조건(KH_CYN.SYS_C0031039)에 위배됩니다 (제약조건 이름 설정ㄴ)
-- ORA-00001: 무결성 제약 조건(KH_CYN.USER_ID_UK)에 위배됩니다   (제약조건 이름 설정ㅇ)
...
-- (참고)오류 보고에 나타나는 SYS_C008635 같은 제약 조건명으로
...
INSERT INTO USER_USED_UK2
VALUES(1, 'user01', 'pass01', '홍길동', '남', '010-1234-5678', 'hong123@kh.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_CYN.USER_ID_NAME_UK)에 위배됩니다
...
--    USER_NO NUMBER PRIMARY KEY, 					    -- 컬럼 레벨(제약조건명 지정ㄴ)
--    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨(제약조건명 지정ㅇ)
	USER_NO NUMBER,
    ...
    -- 테이블 레벨
--    , PRIMARY KEY(USER_NO) -- 제약조건명ㄴ
    , CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO) -- 제약조건명ㅇ
);
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_CYN.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_CYN"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_CYN.PK_USERNO_USERID)에 위배됩니다

SELECT * FROM USER_USED_PK2;

-- PRIMARY KEY는 NULL이 들어갈 수 없음 (복합키인 경우 모든 필드에 null 안됨)
INSERT INTO USER_USED_PK2
VALUES(NULL, 'user01', 'pass01', '신사임당', '여', '010-9999-9999', 'sin123@kh.or.kr');
-- ORA-01400: NULL을 ("KH_CYN"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK1 REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼레벨로 FK 제약조건 설정
);
...
--> NULL 사용 가능. (외래키에서의 NULL은 외부테이블을 참조하지 않는다는 의미이다.)
...
-- ORA-02291: 무결성 제약조건(KH_CYN.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
-- 참조할 값을 제공하는 필드 : 부모
-- 값을 참조하는 필드 : 자식
...
-- ORA-02292: 무결성 제약조건(KH_CYN.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
CREATE TABLE USER_USED_FK2(
  ...
  --      제약조건 이름 설정           참조할 외래키 필드            참조할 테이블              삭제옵션
);
...
SELECT * FROM USER_GRADE2;
SELECT * FROM USER_USED_FK2;
--> GRADE_CODE = 10이었던 필드가 NULL로 변경됨
...
CREATE TABLE USER_USED_FK3(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FK3 REFERENCES USER_GRADE3/*(GRADE_CODE)*/ ON DELETE CASCADE
  --                                              컬럼명 미작성 시 PK로 자동 연결
);
...
SELECT * FROM USER_GRADE3;
SELECT * FROM USER_USED_FK3;
--> USER_GRADE3.GRADE_CODE = 10을 참조하고 있던 USER_USDE_FK3의 행이 모두 삭제됨
...
CREATE TABLE USER_USED_CHECK(
  ...
  USER_PWD VARCHAR2(30) NOT NULL,
  USER_NAME VARCHAR2(30),
  GENDER VARCHAR2(10) CONSTRAINT GENDER_CHECK CHECK(GENDER IN('남', '여')), -- 컬럼레벨
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50)
);
...
-- ORA-02290: 체크 제약조건(KH_CYN.GENDER_CHECK)이 위배되었습니다
...
INSERT INTO USER_TEST VALUES(1, 'USER01', 'PASS01', '990101-222222', '여', '010-1223-4564', '경기도 하남시', 'N');
INSERT INTO USER_TEST VALUES(2, 'USER02', 'PASS02', '980101-111111', '남', '010-2345-3457', '서울시 강동구', 'N');
INSERT INTO USER_TEST VALUES(3, 'USER03', 'PASS03', '970101-222222', '여', '010-5346-2354', '부산시 뫄뫄동', 'N');
INSERT INTO USER_TEST VALUES(4, 'USER04', 'PASS04', '960101-111111', '남', '010-7655-4135', '경기도 구리시', 'Y');
INSERT INTO USER_TEST VALUES(5, 'USER05', 'PASS05', '950101-222222', '여', '010-5767-4578', '서울시 도봉구', 'N');
...
-- 컬럼명, 데이터 타입, 값이 복사되고, 제약조건은 NOT NULL 만 복사됨*** (PK 등 다른 제약조건들은 새로 지정해주어야 함)

-- 1) 테이블 전체 복사
-- CREATE TABLE 테이블명 AS 서브쿼리; --> 서브쿼리의 결과(RESULT SET)의 형태로 테이블 생성
...
-- 3) 테이블의 형태(컬럼명, 데이터타입)만 복사(컬럼 값은 복사X)
CREATE TABLE DEPARTMENT_COPY AS
SELECT * FROM DEPARTMENT WHERE 1 = 0; -- 어떤 행도 조회되지 않도록 WHERE 절 조건문 설정
