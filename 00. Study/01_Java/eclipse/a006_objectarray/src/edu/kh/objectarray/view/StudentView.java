...
public class StudentView {
	...
	private void addStudent() {
		...
		if (result) { // true 인 경우 == 학생 추가 성공
			System.out.println(name + " 학생이 추가되었습니다.");
		} else { // false 인 경우 == 학생 추가 실패
			System.out.println("더 이상 학생을 추가할 수 없습니다.");
		}
	}

	/**
	 * 2. 학생 전체 조회 : service가 참조하는 객체의 필드 중 studentArr를 반환받아 각 배열 요소를 화면에 출력
	 */
	private void selectAll() {
		System.out.println("[ 학생 전체 조회 ]");

		// * 매개변수, 반환값으로 참조변수가 사용될 경우 참조하고 있는 주소가 복사된다. -> 얕은복사
		Student[] studentList = service.selectAll();
		
		for (int i = 0; i < studentList.length; i++) {
			
			// NullPointerException 발생 가능
			// : 참조하고있는 객체가 없는 참조변수를 이용해서 필드/메서드를 호출할 때 발생하는 에러(예외)
			...
			// Student 객체가 가지고있는 toString() 메서드를 호출해서 학생 정보를 문자열로 반환받아 출력
	...
	private void selectIndex() {
		...
		// index에 해당하는 배열 요소를 반환받아 저장
		Student student = service.selectIndex(index);

		// NullPointerException 예외 방지
		if( student == null ) { // 반환받은 index번째 요소가 null인 경우 오류메세지 출력
			System.out.println("해당 인덱스에 학생 정보가 존재하지 않습니다.");
		} else { // 반환받은 index번째 요소가 null이 아닌 경우 학생 정보 출력
	...
	private void selectName() {
		...
		Student[] students = service.selectName(name);
		
		// NullPointerException 예외 방지
		if( students == null ) { // 반환받은 요소가 null인 경우 오류메세지 출력
			System.out.println("이름이 일치하는 학생이 없습니다.");
			return;
		}
		
		// 반환받은 요소가 null이 아닌 경우 학생 정보 출력
		/** 
		 * 향상된 for문
		 * for( 배열에서 꺼낸 요소 하나를 저장할 변수 : 배열명 ) {}
		 * -> 배열에서 꺼내는 순서는 0부터 1씩 증가하여 마지막 인덱스까지 (순차 접근 = 반복 접근 : 모든 요소에 접근)
		 */
		for( Student s : students ) { // s : students의 배열 요소가 반복마다 하나씩 저장되는 변수
			/** 
			 * if / else if / else / for / while 등 
			 * {} 블록을 작성하는 구문에서 {} 블록을 생략하면 다음 한 줄에만 해당 구문을 적용시킨다
			 * 한 줄의 기준은 세미콜론 전까지
			 */
			if( s == null ) break;
	...
	private void updateStudent() {
		...
		// index 번째 학생의 점수를 수정하는 서비스 호출
		// 수정 성공 시 true, 아니면 false 반환
		boolean result = service.updateStudent(index, kor, eng, math);

		// 수정 결과에 따라 출력구문 제어
		if( result ) System.out.println("수정되었습니다.");
		else 		 System.out.println("해당 인덱스에 학생 정보가 존재하지 않습니다.");
	}
