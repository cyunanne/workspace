...
/*
    ...
    *** 정규표현식 객체가 제공하는 메서드(함수) ***
        ...
        2. regEx.exec(문자열) : 문자열이 정규표현식 패턴에 부합하면 첫 번째 매칭되는 문자열을 반환, 없으면 null 반환
...
// 2) 객체에 특정 Key가 존재하지 않으면 추가할 수 있다 (대입연산자)
...
memberEmail.addEventListener("input", () => {
    ...
    if(memberEmail.value.trim().length == 0) {
        memberEmail.value = ""; // 공백 제거(공백 입력 불가)
        emailMessage.innerText = "메일을 받을 수 있는 이메일을 입력해주세요.";
        emailMessage.classList.remove("confirm", "error"); // confirm, error 클래스를 삭제해서 검정 글씨로 만들기
        checkObj.memberEmail = false; // 빈칸==유효X
        return;
    }

    // 정규 표현식을 이용해 유효한 형식인지 판별

    // 1) 정규표현식 객체 생성
    const regEx = /^[A-Za-z\d\-\_]{4,}@[가-힣\w\-\_]+(\.\w+){1,3}$/;

    // 2) 입력받은 이메일과 정규식의 일치 여부 판별
    if(regEx.test(memberEmail.value)) { // 유효한 경우
        ...
        .then(count => {
            ...
            if(count == 0) {
                ...
                emailMessage.classList.remove("confrim"); // .error 스타일 제거
...
memberPw.addEventListener('input', e => {
    ...
    if(memberPw.value.trim().length == 0) {
        memberPw.value = ""; // 공백 제거(공백 입력 불가)
        pwMessage.innerText = "영어,숫자,특수문자(!,@,#,-,_) 6~20글자 사이로 입력해주세요.";
        pwMessage.classList.remove("confirm", "error"); // confirm, error 클래스를 삭제해서 검정 글씨로 만들기
        checkObj.memberPw = false; // 빈칸==유효X
        return;
    }

    // 정규 표현식을 이요한 비밀번호 유효성 검사
...
memberNickname.addEventListener("input", () => {
    ...
    if(memberNickname.value.trim() == '') {
        ...
        memberNickname.value = ""; // 빈칸 입력 못하게 막기
        return;
    }
...
document.getElementById('signUpFrm').addEventListener('submit', e => {
    // checkObj의 모든 value가 true인지 검사
    
    // (배열용 for문)
    // for ... of : 향상된 for문
    // -> itorator 속성을 지닌 배열, 유사배열 사용 가능
    ...
    for(let key in checkObj) {
        if(!checkObj[key]) {
            // 각 key에 대한 value(true/false)를 얻어옴
            // false인 경우 == 유효하지 않다
...
checkAuthKeyBtn.addEventListener("click", function(){
    if(authMin > 0 || authSec > 0){ // 시간 제한이 지나지 않은 경우에만 인증번호 검사 진행
        ...
        const query = new URLSearchParams(obj).toString() // => 'inputKey=123456&email=user@example.com' 보내야 할 파라미터가 많을 때 활용
