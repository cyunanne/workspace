...
<body>
    <main>
        ...
        <section class="board-detail">  
            ...
            <c:if test="${not empty board.imageList}">
                ...
                <!-- 썸네일 영역(썸네일이 있을 경우) -->
                <%-- 
                    - 이미지는 IMG_ORDER 오름차순으로 정렬
                    - IMG_ORDER=0인 이미지가 썸네일이다
                    ---> imageList에 썸네일이 있다면 
                        조회되었을 때 IMG_ORDER=0인 이미지가 
                        imageList[0]에 저장되어있을 것이다.
                --%>
                ...
                <c:if test="${fn:length(board.imageList) > start}">
                    ...
                    <h5>업로드 이미지</h5>
                    <div class="img-box">
                        <c:forEach var="i" begin="${start}" end="${fn:length(board.imageList)-1}" step="1">
    ...
    <%--
        ...
        3) jsp 파일 제일 위에 있는 script 태그에 JS+EL 이용해서 전역 번수로 선언하기
    --%>
