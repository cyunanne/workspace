...
//	@Before("execution(* edu.kh.project..*Impl*.*(..))") // 타겟 수행 전
	...
	@Before("CommonPointcut.serviceImplPointcut()") // 지정된 클래스.메서드에 작성된 @Pointcut() 어노테이션의 내용을 타겟으로 삼음
	public void beforeLog(JoinPoint jp) { // Advice(수행할 코드)
		// JoinPoint jp : AOP의 부가기능이 적용된 대상
		//                객체, 메서드, 파라미터 정보를 얻을 수 있게 해주는 객체
		
		String className = jp.getTarget().getClass().getSimpleName(); // 대상객체의 간단한 클래스명(패키지명 제외)
		String methodName = jp.getSignature().getName(); // 메서드 선언부(메서드 시그니처)에서 메서드명만 얻어옴
