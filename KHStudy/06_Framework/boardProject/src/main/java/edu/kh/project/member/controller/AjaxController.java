...
//	public @ResponseBody String selectMemberTel(String nickname) {
//  이렇게도 쓸 수 있는데 가독성이 나쁘니까 그냥 위에 쓰셈
		
		// return redirect OR forward -> 새로운 화면이 보임(동기식)
		// return data; -> 데이터를 요청한 곳으로 반환(비동기식)
	...
	@ResponseBody // HttpMessageConverter를 이용해 js에서 인식할 수 있는 형태(text/json)로 변환
				  // + 비동기 요청한 곳으로 돌아감
	...
	public int checkNickname(String nickname) {
		return service.checkNickname(nickname); // String으로 변환해서 반환
	}
	
	// 이메일로 회원 정보 조회
	@PostMapping(value="/selectMember", produces="application/json; charset=UTF-8")
	@ResponseBody // Java데이터 -> JSON, TEXT로 변환 + 비동기 요청한 곳으로 응답
