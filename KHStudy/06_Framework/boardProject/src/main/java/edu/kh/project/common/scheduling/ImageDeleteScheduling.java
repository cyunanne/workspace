...
//	@Scheduled(fixedDelay = 10000) // fixedDelay : 작업 종료 후 일정시간(ms단위) 대기 (작업시간만큼 추가대기)
//	@Scheduled(fixedRate = 10000) // fixedRate : 스케줄러 시작 후 일정시간마다 반복 (작업시간 무시)
...
//		System.out.println("스케줄러가 일정 시간마다 자동으로 출력");
		...
		// 서버에 저장된 파일 목록을 조회해서
		// DB에 저장된 파일 목록과 비교하여 매칭되지 않는 서버 파일 제거
		
		// 1. 서버에 저장된 파일 목록 조회
		String filePath = servletContext.getRealPath("/resources/images/board");
		// 실제 서버 위치 : D:\workspace\KHStudy\06_Framework\boardProject\src\main\webapp\resources\images\board
...
//				String s = temp[temp.length-1];
				...
				// List.indexOf(객체) = 객체가 List에 있으면 해당 인덱스를 반환하고, 없으면 -1 반환
				if(dbImageList.indexOf(server.getName()) == -1) {
				// DB 파일 목록         서버 파일 이름     없음
