...
<body>
    ...
    <pre>
        - JSP의 표현식<%-- <%= expression%> --%>을 조금 더 효율적이고 간단히 작성할 수 있도록 고안된 언어로, JSP에 내장되어있다
        - 화면에 출력하고자 하는 자바 코드를 \%{ key } <%-- \$로 작성하면 화면에 $로 보임 --%> 형식으로 작성하면
    ...
    테스트 1(JSP 표현식) : <%= request.getParameter("test") %><br>
    테스트 2(EL) : ${param.test}

    <hr>

    <h3>EL의 특징 2번 : null 이나 NullPointerException을 빈칸으로 처리한다</h3>
    
    테스트 1(JSP 표현식) : <%= request.getParameter("num") %><br>
