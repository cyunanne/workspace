...
--> DEPT_ID, LOCATION_ID 에만 NOT NULL 제약조건이 설정되어있는 상태
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT UK_DEPT_TITLE UNIQUE(DEPT_TITLE);

-- DEPT_COPY 테이블의 LOCATION_ID 컬럼에 CHECK 제약조건 추가
-- 컬럼에 작성할 수 있는 값은 L1, L2, L3, L4, L5 
-- 제약조건명 : LOCATION_ID_CHK
ALTER TABLE DEPT_COPY ADD CONSTRAINT CHK_LOCATION_ID CHECK(LOCATION_ID IN('L1', 'L2', 'L3', 'L4', 'L5'));
...
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE NOT NULL; -- **** NULL 관련 설정만 MODIFY 사용 ***

---------------------------

-- DEPT_COPY에 추가한 제약조건 중 PK 빼고 모두 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT CHK_LOCATION_ID;
ALTER TABLE DEPT_COPY DROP CONSTRAINT UK_DEPT_TITLE;
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C0031215; -- NOT NULL 제약조건 ADD는 안되는데 DROP은 똑같이 됨 

-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE /*CONSTRAINT SYS_C0031215*/ NULL; -- 제약조건 이름 안써도 됨
...
--> 새로 추가된 컬럼 LNAME에 기본값 '한국'이 자동 삽
...
--> 기존에 저장된 값이 수정하려는 데이터 타입보다 크면 수정할 수 없다.
