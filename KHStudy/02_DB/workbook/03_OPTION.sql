...
-- 5. 2004년 2학기에 'C3118100' 과목을 수강한 학생들의 학점을 조회하려고 한다. 학점이 높은 학생부터 표시하고,
-- 학점이 같으면 학번이 낮은 학생부터 표시하는 구문을 작성해 보시오.
-- (워크북 결과와 동일하게 소수점 둘째 자리까지 0으로 표현하기 위해서 TO_CHAR(NUMBER, 'FM9.00') 포맷 사용)
SELECT STUDENT_NO, TO_CHAR(AVG(POINT), 'FM9.00') 학점
FROM TB_GRADE
WHERE STUDENT_NO IN (
...
JOIN TB_DEPARTMENT USING(DEPARTMENT_NO)
JOIN (
	SELECT STUDENT_NO, AVG(POINT) 평점
	FROM TB_GRADE
	GROUP BY STUDENT_NO
	HAVING AVG(POINT) >= 4.0) USING(STUDENT_NO)
WHERE ABSENCE_YN = 'N'
ORDER BY 학번;
-- #2.
