...
// 내 정보 수정 form 태그가 존재할 때 (내 정보 페이지일 때)
if(updateInfo != null) {
...
const secessionFrm = document.getElementById("sessesionFrm");
if(secessionFrm != null) { // 탈퇴 페이지인 경우
    const memberPw = document.getElementById("memberPw");
    const agree = document.getElementById("agree");

    secessionFrm.addEventListener("submit", e => { // 비밀번호 미작성
...
const imageInput = document.getElementById("imageInput"); // input 버튼
const deleteImage = document.getElementById("deleteImage"); // x 버튼

// 초기 프로필 이미지 상태 저장
// => true==이전업로드이미지, false==기본이미지
let initCheck; 

// 프로필 이미지가 새로 업로드되거나 삭제 되었음을 나타내는 변수
...
if(imageInput != null) { // 화면에 imageInput이 있을 경우
    ...
    imageInput.addEventListener("change", e => {
        ...
        const maxSize = 1 * 1024 * 1024 * 2; // 파일 최대 크기 지정(바이트 단위) : 2MB
        ...
        if(file == undefined) { 
            ...
            profileImage.setAttribute("src", originalImage); // 취소 시 기존 프로필 이미지로 변경
            deleteCheck = -1;
            return;
        }
    ...
    deleteImage.addEventListener('click', () => {
        ...
        profileImage.setAttribute("src", "/resources/images/user.png"); // 프로필 이미지를 기본 이미지로 변경
        deleteCheck = 0;
    });
