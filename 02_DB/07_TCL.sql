...
-- DEPARTMENT2 테이블에 D0, 경리부, L2 삽입
INSERT INTO DEPARTMENT2 VALUES ('D0', '경리부', 'L2');

-- 경리부 삽입 확인
SELECT * FROM DEPARTMENT2;
--> 잘 된 것처럼 보이지만 아직 DB에 실제로 삽입되지 않았다.
...
-- 다시 한 번 DEPARTMENT2 테이블에 D0, 경리부, L2 삽입
INSERT INTO DEPARTMENT2 VALUES ('D0', '경리부', 'L2');
...
ROLLBACK; 
--> 이미 앞서서 COMMIT을 수행해서 돌아갈 내용이 없음
SELECT * FROM DEPARTMENT2;
--> 경리부 남아있음 확인
...
--UPDATE DEPARTMENT2 SET DEPT_ID = 'D1' WHERE DEPT_ID = 'D0';
ROLLBACK;