...
-- 3.
SELECT PROFESSOR_NAME 교수이름, FLOOR((SYSDATE - TO_DATE('19'||SUBSTR(PROFESSOR_SSN, 1, 6))) / 365) 나이
FROM TB_PROFESSOR
...
-- 13.
SELECT DISTINCT DEPARTMENT_NO 학과코드명, NVL("휴학생 수", 0) "휴학생 수"
FROM TB_STUDENT
LEFT JOIN (
	SELECT DEPARTMENT_NO, COUNT(*) "휴학생 수"
	FROM TB_STUDENT
	WHERE ABSENCE_YN = 'Y'
	GROUP BY DEPARTMENT_NO
) USING (DEPARTMENT_NO)
